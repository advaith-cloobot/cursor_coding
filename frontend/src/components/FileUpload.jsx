...
const FileUpload = ({ assetId, onFilesUploaded, existingFiles = [] }) => {
  ...
  const handleSetPrimary = async (fileId) => {
    try {
      const { setPrimaryImage } = await import('../services/api');
  ...
  const handleDeleteFile = async (fileId) => {
    ...
    try {
      const { deleteFile: deleteFileApi, getFileUrl } = await import('../services/api');
              ...
              <Grid item xs={6} sm={4} md={3} key={file.id}>
                ...
                >
                  {file.file_type === 'image' ? (
                    <Box
                      component="img"
                      src={file.file_path.startsWith('http') ? file.file_path : `http://localhost:5000/api/uploads/${file.file_path.replace(/.*uploads[\\/]/, '').replace(/\\/g, '/')}`}
                      alt={file.file_name}
                      sx={{
                        width: '100%',
                        height: 150,
