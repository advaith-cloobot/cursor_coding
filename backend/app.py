...
def get_lockers():
    ...
    try:
        ...
        lockers_query = '''
            ...
            FROM lockers l
            LEFT JOIN assets a ON l.id = a.locker_id
            GROUP BY l.id
            ORDER BY l.created_at DESC
        '''
        lockers = conn.execute(lockers_query).fetchall()
        
        # Separate into two groups
        lockers_with_withdrawn = []
        lockers_intact = []
        
        for locker in lockers:
            locker_dict = row_to_dict(locker)
            withdrawn_count = locker_dict.get('withdrawn_assets', 0) or 0
            total_count = locker_dict.get('total_assets', 0) or 0
            
            if withdrawn_count > 0:
                lockers_with_withdrawn.append(locker_dict)
            else:
                lockers_intact.append(locker_dict)
        
        conn.close()
        
        return jsonify({
            'withdrawn': lockers_with_withdrawn,
            'intact': lockers_intact
        }), 200
...
def update_asset(asset_id):
    ...
    try:
        ...
        # Create edit log entry if there are changes (skip initial creation)
        if edited_fields:
            cursor = conn.cursor()
            cursor.execute(
                '''INSERT INTO asset_edit_log 
...
def get_asset_files(asset_id):
    ...
    try:
        ...
        return jsonify([row_to_dict(file) for file in files]), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def delete_file(file_id):
    ...
    try:
        ...
        is_primary = file_dict['is_primary']
        ...
        # If this was primary, update asset
        if is_primary:
            # Set next image as primary if available
...
def create_transaction():
    ...
    try:
        ...
        # Fetch transaction with asset info
        transaction = conn.execute(
            '''SELECT t.*, a.name as asset_name, a.asset_type
               FROM transactions t
               JOIN assets a ON t.asset_id = a.id
...
def update_transaction(transaction_id):
    ...
    try:
        ...
        # Update transaction
        transaction_type = data.get('transaction_type', old_type)
        if transaction_type not in ['depositing', 'withdrawing', 'permanently_remove']:
            conn.close()
        ...
        conn.execute(
            ...
            (
                ...
                data.get('reason'),
                data.get('responsible_person'),
                data.get('transaction_date', transaction['transaction_date']),
                transaction_id
            )
        )
        ...
        if transaction_type != old_type:
            ...
            new_status = status_map[transaction_type]
            conn.execute(
                'UPDATE assets SET status = ? WHERE id = ?',
                (new_status, asset_id)
            )
...
def delete_transaction(transaction_id):
    ...
    try:
        ...
        # Update asset status based on latest transaction
        ...
        if latest_transaction:
            ...
            # No transactions left, default to deposited
